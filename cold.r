...
cold <- function()
{
  ...
  cold_data <- read.table("Data/coldstress.txt",header=T,sep="\t",row.names=1,stringsAsFactors=F)
  control_data <- read.table("Data/control.txt",header=T,sep="\t",row.names=1,stringsAsFactors=F)
  
  exp_val <- cbind(control_data, cold_data)
  ...
  pr_cold <- (exp(bstat) / (matrix(1,nrow(bstat),ncol(bstat))+exp(bstat)))
  
  write.table(pr_cold,"Data/pr_cold.txt",row.names=T,col.names=T,sep="\t")
  
  data <- sapply(paste("t",tps,sep=""),function(name,x){apply(x[,which(colnames(x)==name)],1,median)},exp_val)
  write.table(data,"Data/cold.txt",row.names=T,col.names=T,sep="\t")
